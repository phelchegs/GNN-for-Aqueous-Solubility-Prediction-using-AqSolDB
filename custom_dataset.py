...
class CustomDataset(Dataset):
    ...
    def process(self):
        ...
        for raw_path in [os.path.join(self.root, file) for file in self.raw_file_paths]:
            ...
            bins, labels = self.classifier()
            self.data['Sol_Label'] = pd.cut(self.data['Solubility'], bins = bins, labels = labels, right = True)
            for index, row in tqdm.tqdm(self.data.iterrows(), total = self.data.shape[0], desc = "Processing rows"):
                g = mol_converter.featurize(row['SMILES'])
